...
#读取车子数据
def readTxt():
    data = np.loadtxt('car_xytheta_dot_record3.txt')
    xy_path = data[:,0:2]
...
if __name__ == "__main__":
    ...
    for ii in range(1,len(xy_path)+1):
        i = ii/down_times
        arm_xyztheta_path_noise.append([0.0015*i,-0.0015*i,-0.10*np.cos(2*3.14159/100*i),0.001*i] + 0.00001*np.random.normal(0,2,4))
        arm_xyztheta_path.append([0.0015*i,-0.0015*i,-0.10*np.cos(2*3.14159/100*i),0.001*i])
    ...
    car_gpr.plot_field(-1,2,-1,2,xy_path)
    # plt.figure()
    ...
    rtde_c.speedJ(joint_vel.ravel(),1,0)
    time.sleep(1)
    rtde_c.stopJ()
