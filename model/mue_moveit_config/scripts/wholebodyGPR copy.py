...
class MyGPR():
    def __init__(self,data_in,data_out,normalize = False,n_restarts_optimizer=5):
        ...
        kernel1 =  10*RBF(length_scale=1e-3, length_scale_bounds=(1e-8, 1e2))+0.000001*WhiteKernel(noise_level = 1e-7,noise_level_bounds=(1e-15,1e-2))
...
def eulerAnglesToRotationMatrix(theta) :
    ...
    return R
#输入一个3*4的T阵，和相对于T阵的变换，输出一个新的T阵 用np矩阵的形式
def change_T(initT,change):
    changeT = np.zeros((3,4))
    changeT[0,3] = initT[0,3]+change[0]
    changeT[1,3] = initT[1,3]+change[1]
    changeT[2,3] = initT[2,3]+change[2]
    initR = initT[0:3,0:3]
    changeR = np.matmul(eulerAnglesToRotationMatrix([0,change[3],0]),initR)
    changeT[0:3,0:3] = changeR
    return changeT
...
if __name__ == "__main__":
    ...
    xy_dot_path_filtered_down = down_sample(xy_dot_path_filtered,8)
    xy_path_down = down_sample(xy_path,8)
    #机械臂轨迹生成
    arm_xyztheta_path_noise = [] 
    arm_xyztheta_path = [] 
    for i in range(1,len(xy_path_down)+1):
        arm_xyztheta_path_noise.append([0.0015*i,-0.0015*i,-0.10*np.cos(2*3.14159/200*i),0.001*i] + 0.00001*np.random.normal(0,2,4))
        arm_xyztheta_path.append([0.0015*i,-0.0015*i,-0.15*np.cos(2*3.14159/200*i),0.001*i])
    ...
    car2arm_gpr = MyGPR(xy_path_down,arm_xyztheta_path,normalize=True)
    ...
    xxx,yyy = np.meshgrid(xx,yy)
    predict_point = np.concatenate((xxx.reshape(-1,1),yyy.reshape(-1,1)),1)
    arm_xyztheta_path_all = car2arm_gpr.gpr1.predict(predict_point,return_std=False)
    ...
    # car2arm_gpr = MyGPR(xy_path_down,arm_xyztheta_path)


    arm2car_gpr = MyGPR(arm_xyztheta_path_all,predict_point,normalize=False,n_restarts_optimizer=0)
    t0 = time.time()
    # car2arm_gpr.gpr1.predict(np.array((current_x,current_y)).reshape(1,-1))
    t0 = time.time()
    ############测试内容
    ...
    yy = np.arange(ymin,ymax,y_step)
    xxx,yyy = np.meshgrid(xx,yy)
    predict_point1 = np.concatenate((xxx.reshape(-1,1),yyy.reshape(-1,1)),1)
    time.sleep(1)
    arm_xyztheta_path_all1 = car2arm_gpr.gpr1.predict(predict_point1,return_std=False)
    ...
    result2 = car2arm_gpr.gpr1.predict(xy_path_down+np.array((1,0)),return_std=False)
    
    plt.plot(result1[:,0]-result2[:,0],result1[:,1]-result2[:,1])
    plt.show()
...
####################################################################################################################################################
    while True:
        ...
        if step < 0:#给扰动
            ...
            if (np.abs(delta_theta*57.3) > 5 or np.sqrt(result[0,0]**2+result[0,1]**2) > 1) and bz_flag == False:        #假如角度相差不大，就直接走，否则走Bezier
                bz_flag = True
                bezier = BEZIER()
            ...
            if bz_flag ==True:
                ...
                vel.linear.x = np.sqrt(result[0,0]**2+result[0,1]**2)
...
#########################################################################################################################################        
        ...
        #要减去初始值才是输入GPR的输入吧
        coordinate_result = arm2car_gpr.gpr1.predict(np.array((end_state[0]-end_init_state[0],end_state[1]-end_init_state[1],end_state[2]-end_init_state[2],end_state[5]-end_init_state[5])).reshape(1,-1))
        ...
        if(distance > 999):
            ...
            if(abs(atan2(coordinate_result[0,1] - current_y,coordinate_result[0,0] - current_x) - current_theta) < 20/57.3):#正着走
                print('coor1')
                vel.linear.x = distance*5
                vel.angular.z = (np.arctan2(result[0,1],result[0,0])-current_theta)*5.2    #10.2是由延时0.1s决定的
                base_pub.publish(vel)
                time.sleep(0.2)
                vel.linear.x = 0
                vel.angular.z = 0
                base_pub.publish(vel)
            elif(abs(atan2(-coordinate_result[0,1] + current_y,-coordinate_result[0,0] + current_x) - current_theta) < 20/57.3):#反着走   
                vel.linear.x = 0
                vel.angular.z = 0
                base_pub.publish(vel)
                time.sleep(0.5)
                ...
                current_x,current_y,current_theta = car_sub_record[-1][0],car_sub_record[-1][1],car_sub_record[-1][2]
                coordinate_result = arm2car_gpr.gpr1.predict(np.array((end_state[0]-end_init_state[0],end_state[1]-end_init_state[1],end_state[2]-end_init_state[2],end_state[5]-end_init_state[5])).reshape(1,-1))
                print('coor2,before',distance,'  after',np.sqrt((current_x-coordinate_result[0,0])**2+(current_y-coordinate_result[0,1])**2))
    ...
    plt.figure()
    plt.plot([x[0]-ur_end_record[0][0] for x in ur_end_record])
    plt.plot([x[1]-ur_end_record[0][1] for x in ur_end_record])
    plt.plot([x[2]-ur_end_record[0][2] for x in ur_end_record])
    plt.plot([x[5]-ur_end_record[0][5] for x in ur_end_record])
    plt.plot([x[0] for x in arm_xyztheta_path_noise])
    plt.plot([x[1] for x in arm_xyztheta_path_noise])
    plt.plot([x[2] for x in arm_xyztheta_path_noise])
    plt.plot([x[3] for x in arm_xyztheta_path_noise])
