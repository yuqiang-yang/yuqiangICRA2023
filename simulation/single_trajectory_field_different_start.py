...
if __name__ == "__main__":
    ...
    #         if np.sqrt((current_x-2.9)**2+(current_y-2.9)**2) < 0.2 and last_distance_to_end < np.linalg.norm(np.array((current_x,current_y))-xy_path[-1],axis=0):
    #             break
    #         t0 = time.time()
    #         if step >= 15000:
    #             break
    ...
    #     np.savetxt('data/2multistart'+str(cnt)+'.txt',np.array(gazebo_sub_record))
...
###################
    ...
    #     while True:
    #         #if the car is near the target, break the loop
    #         if (np.sqrt((current_x-xy_path1[-1,0])**2+(current_y-xy_path1[-1,1])**2) < 0.2 or np.sqrt((current_x-xy_path2[-1,0])**2+(current_y-xy_path2[-1,1])**2) < 0.2 ) and (last_distance_to_end < np.linalg.norm(np.array((current_x,current_y))-xy_path[-1],axis=0) or last_distance_to_end < np.linalg.norm(np.array((current_x,current_y))-xy_path1[-1],axis=0)):
    #             break
    #         t0 = time.time()
    #         if step >= 7000:
    #             break
    ...
    #     time.sleep(0.2)
    #     plt.plot([x[0] for x in gazebo_sub_record],[x[1] for x in gazebo_sub_record])
    #     np.savetxt('data/3multistart_mul'+str(cnt)+'.txt',np.array(gazebo_sub_record))
    # plt.plot(xy_path[:,0],xy_path[:,1])
    # plt.legend(('1','2','3','4','5','6','7'))
    #np.savetxt('data/2multistartref.txt',np.array(gazebo_sub_record))
...
###
    ...
    #         print('current',current_theta,'target',target_angle,'delta',delta_theta)
    #         if len(car_sub_record) == 0:
    #             #print('w, zero')
    #             #base_sub = rospy.Subscriber("/mobile_base_controller/odom",Odometry,car_sub_callback)
    #             w0 = 0
    ...
    #             vel.linear.x = np.sqrt(result[0,0]**2+result[0,1]**2)*0.25
    ...
    #              vel.angular.z = np.sign(vel.angular.z)*0.1
    ...
    #     time.sleep(0.2)
    #     plt.plot([x[0] for x in gazebo_sub_record],[x[1] for x in gazebo_sub_record])
    #     np.savetxt('data/5multistart_jj'+str(cnt)+'.txt',np.array(gazebo_sub_record))
    # plt.plot(xy_path[:,0],xy_path[:,1])
    # plt.legend(('1','2','3','4','5','6','7'))
    # np.savetxt('data/5multistartref_jj.txt',np.array(gazebo_sub_record))
    ...
    for st in start_points:
        ...
        while True:
            ...
            if bz_flag ==True:
                ...
                vel.linear.x = np.sqrt(result[0,0]**2+result[0,1]**2)*0.3
                acc,_ = time_optimal_for_1D_tracking(delta_theta, 0, 0, w0, car_angular_acc)
                vel.linear.x = 0
                if delta_theta < 0.1:
                    bz_flag = False
        ...
        time.sleep(0.2)
        plt.plot([x[0] for x in gazebo_sub_record],[x[1] for x in gazebo_sub_record])
        np.savetxt('data/5multistart_mul_jj'+str(cnt)+'.txt',np.array(gazebo_sub_record))
    plt.plot(xy_path[:,0],xy_path[:,1])
    plt.legend(('1','2','3','4','5','6','7'))
    #np.savetxt('data/2multistartref.txt',np.array(gazebo_sub_record))