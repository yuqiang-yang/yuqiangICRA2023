...
class MyGPR():
    ...
    def get_next_step_jens(self,predict_point):
        ...
        return np.array((result[0,0] - (self.diff_std[remap_i,remap_j,0])*3 ,  result[0,1] - (self.diff_std[remap_i,remap_j,1])*3)).reshape(1,-1)
    ...
    def plot_field1(self,xmin,xmax,ymin,ymax,demonstrate_point,step=75,field_density = 7):
        ...
        vmax = np.max(varian_res)/0.8
        vmin = np.min(varian_res)/8

        #plt.figure()
        #plt.subplot(234)
        #plt.pcolormesh(xxx1,yyy1,self.std_matrix,cmap='Blues',alpha=0.5)
        # plt.xlim((xmin,xmax))
        # plt.ylim((ymin,ymax))
        ...
        half = int(demonstrate_point.shape[0]/2)
        color = 'yellow'
        color1= 'seashell'
        # for i in range(field_density**2):
        #     plt.arrow(predict_point[i,0],predict_point[i,1],result[i,0],result[i,1],head_width=0.05,color=color1)#seashell
 
        # plt.plot(demonstrate_point[:half-5,0],demonstrate_point[:half-5,1],linewidth=4,color=color,alpha=0.5)
        # plt.plot(demonstrate_point[half+5:,0],demonstrate_point[half+5:,1],linewidth=4,color=color,alpha=0.5)        
        plt.subplot(143)
        plt.pcolormesh(xxx1[:-1,:-1],yyy1[:-1,:-1],np.sqrt(abs(diff_std[:-1,:-1,0])**2+abs(diff_std[:-1,:-1,1])**2)*6,cmap=light_jet,alpha=1,shading='gouraud')
        for i in range(field_density**2): 
            ...
            res*=2
            plt.arrow(predict_point[i,0],predict_point[i,1], res[0,0], res[0,1],head_width=0.1,color='purple')     
        
        plt.plot(demonstrate_point[:half-5,0],demonstrate_point[:half-5,1],linewidth=4,color=color,alpha=0.7)
        plt.plot(demonstrate_point[half+5:,0],demonstrate_point[half+5:,1],linewidth=4,color=color,alpha=0.7)
        fontdict={'family':'Times New Roman','size':14}
        plt.xticks(fontproperties='Times New Roman',size=14)
        plt.yticks([],fontproperties='Times New Roman',size=14)
        plt.title('(c)',position=(0.07,0.85))
        plt.subplot(144)
        plt.pcolormesh(xxx1,yyy1,varian_res,cmap=light_jet,shading='gouraud')
        for i in range(field_density**2): 
            remap_i = int(i//field_density) * int(step//field_density)
            remap_j = int(i%field_density) * int(step//field_density)
        ...
        plt.xticks(fontproperties='Times New Roman',size=14)
        plt.yticks([],fontproperties='Times New Roman',size=14)
        plt.title('(d)',position=(0.07,0.85))
    def plot_field(self,xmin,xmax,ymin,ymax,demonstrate_point,step=75,field_density = 7):
        ...
        #surf = ax.plot_surface(xxx1,yyy1,self.std_matrix,cmap=cm.coolwarm,linewidth=0,antialiased=False)
        #plt.scatter(predict_point[:,0],predict_point[:,1])
        light_jet = cmap_map(lambda x: x/2 +0.5, matplotlib.cm.coolwarm)
        ...
        #print(predict_point)
        for i in range(field_density**2):
            plt.arrow(predict_point[i,0],predict_point[i,1],result[i,0]*2,result[i,1]*2,head_width=0.1,color='purple')#seashell
        plt.plot(demonstrate_point[:,0],demonstrate_point[:,1],linewidth=4,color='yellow',alpha=0.7)
        fontdict={'family':'Times New Roman','size':14}
        ...
        c.ax.set_yticklabels(['0','0.1'],family='Time New Roman')        #plt.contourf(xxx1,yyy1,self.std_matrix)    
        plt.savefig('IMAGE/tttn.png',bbox_inches='tight',dpi=200)
        ...
        plt.ylim((ymin,ymax))    
        plt.subplot(141)
        print(np.sqrt(abs(diff_std[:-1,:-1,0])**2+abs(diff_std[:-1,:-1,1])**2).shape)
        print(xxx1.shape)
        plt.pcolormesh(xxx1[:-1,:-1],yyy1[:-1,:-1],np.sqrt(abs(diff_std[:-1,:-1,0])**2+abs(diff_std[:-1,:-1,1])**2)*6,cmap=light_jet,shading='gouraud')
        for i in range(field_density**2): 
            remap_i = int(i//field_density) * int(step//field_density)
            remap_j = int(i%field_density) * int(step//field_density)
            res = self.get_next_step_jens(predict_point[i,:].reshape(1,-1))
        ...
        plt.xticks(fontproperties='Times New Roman',size=14)
        plt.yticks(fontproperties='Times New Roman',size=14)        
        plt.title('(a)',position=(0.07,0.85))
        plt.subplot(142)
        ...
        plt.xticks(fontproperties='Times New Roman',size=14)
        plt.yticks([],fontproperties='Times New Roman',size=14)
        plt.title('(b)',position=(0.07,0.85))
        p.subplots_adjust(left=0.07,right=0.96,bottom=0.25,wspace=0.02)
        cax=p.add_axes([0.2,0.1,0.6,0.05])
...
if __name__ == "__main__":
    ...
    gpr = MyGPR(xy_path_down,xy_dot_path_filtered_down)
    gpr.plot_field1(-2,5,-2,5,xy_path)

    p.savefig('IMAGE/ttt1.png',bbox_inches='tight',dpi=200)
    #plt.show()
